...
describe('ViewInfoPage', () => {
  it('renders view tree snapshot', async () => {
    ...
    await waitFor(() => expect(screen.getByText('视图检查器')).toBeTruthy())
    ...
    expect(screen.getByRole('button', { name: '显示连线' })).toBeTruthy()
    fireEvent.click(screen.getByRole('button', { name: '显示连线' }))
    fireEvent.click(screen.getByRole('button', { name: '显示连线' }))
  ...
  it('shows error for invalid client key', async () => {
    ...
    expect(screen.getByText('无效客户端标识，无法查看视图信息。')).toBeTruthy()
  ...
  it('renders replay mode without inspector chrome', async () => {
    ...
    expect(screen.queryByText('视图信息 · ios / mock.tree.snapshot.ios')).toBeNull()
    expect(screen.queryByText('视图检查器')).toBeNull()
